...
# partition tables it keeps for itself. That is the only database this stack
# has: the ingested data lives in the object store, not in postgres.
#
...
# whole reason this job lives in this stack rather than in the postgres stack;
# see that stack's README.md, which prescribes exactly this.
...
for attempt in $(seq 1 30); do
  ...
  if (( attempt == 30 )); then
    ...
    echo "provision: is the postgres stack up? (stacks --vm odsc up postgres)" >&2
    exit 1
  fi
  sleep 2
done
