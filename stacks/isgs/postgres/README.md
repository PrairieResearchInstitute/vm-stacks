...
PostgreSQL 18 for the isgs VM's applications. Reachable only from the private
...
```sh
stacks --vm isgs edit-secrets postgres
```
...
SQL
stacks --vm isgs edit-secrets postgres    # match the secret to what you just set
stacks --vm isgs up postgres              # `up`, not `restart` -- see docs/secrets.md
```
...
```sh
stacks --vm isgs logs postgres -f
stacks --vm isgs status
